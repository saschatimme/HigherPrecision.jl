...
Base.ldexp(a::DoubleFloat64, exp::Int) = DoubleFloat64(ldexp(a.hi, exp), ldexp(a.lo, exp))
"""
	mul_pwr2(a::DoubleFloat64, b::Float64)

`a` * `b`,  where `b` is a power of 2.
"""
mul_pwr2(a::DoubleFloat64, b::Float64) = DoubleFloat64(a.hi * b, a.lo * b)

function Base.exp(a::DoubleFloat64)
...
function Base.log(a::DoubleFloat64)
	...
	x = DoubleFloat64(log(a.hi)) # Initial approximation

	x + a * exp(-x) - 1.0
end


const sin_table = [DoubleFloat64(sin(k * big(π) * 0.0625)) for k = 1:4]
const cos_table = [DoubleFloat64(cos(k * big(π) * 0.0625)) for k = 1:4]


"""
	sin_taylor(a::DoubleFloat64)

Computes sin(a) using Taylor series. Assumes |a| <= π/32.
"""
function sin_taylor(a::DoubleFloat64)
...
"""
	cos_taylor(a::DoubleFloat64)

Computes cos(a) using Taylor series. Assumes |a| <= π/32.
"""
function cos_taylor(a::DoubleFloat64)
...
function sincos_taylor(a::DoubleFloat64)
	if iszero(a)
		zero(a), one(a)
	end
...
function Base.sin(a::DoubleFloat64)
...
function Base.cos(a::DoubleFloat64)
	if iszero(a)
		return one(a)
	end
...
function Base.atan2(y::DoubleFloat64, x::DoubleFloat64)
...
function Base.asin(a::DoubleFloat64)
	abs_a = abs(a)
...
function Base.acos(a::DoubleFloat64)
	abs_a = abs(a)
...
function Base.sinh(a::DoubleFloat64)
	if iszero(a)
		return zero(a)
	end
	if a > 0.05 || a < -0.05
...
function Base.cosh(a::DoubleFloat64)
	if iszero(a)
		return one(a)
	end
	ea = exp(a)
...
function Base.tanh(a::DoubleFloat64)
	if iszero(a)
		return zero(a)
	end
	if abs(convert(Float64, a)) > 0.05
